#!/usr/bin/env bash
# Build a distributable, installable MCP bundle (.mcpb) for the Tesla connector.
#
# Produces tesla.mcpb containing manifest.json, the compiled server/, the
# dashboard public/, package.json, and production node_modules — so it runs
# without an npm install on the target machine.
#
# Install by opening the file in Claude Desktop, or:
#   npx @anthropic-ai/mcpb install tesla.mcpb
set -euo pipefail
