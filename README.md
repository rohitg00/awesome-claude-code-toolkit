...
| [clear-decisions](https://github.com/SathiaAI/clear-decisions) | `git clone https://github.com/SathiaAI/clear-decisions ~/.claude/skills/clear-decisions` | Forces plain-English, unbiased decision support: real options, genuinely fair pros/cons for each (including checks against favoring the AI's own maker or whatever's just more popular), one crisp recommendation with a confidence tag, and a professional-advice disclaimer when money/legal/medical stakes are involved. Includes a harness-agnostic PORTABLE-RULES.md for tools without native skill support. CC BY-SA 4.0. |
