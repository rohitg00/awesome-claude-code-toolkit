...
| [cowork-to-code-bridge](https://github.com/abhinaykrupa/cowork-to-code-bridge) | `curl -fsSL https://raw.githubusercontent.com/abhinaykrupa/cowork-to-code-bridge/main/install.sh \| bash` | Run Claude Code on your real Mac/Linux box from a Claude Cowork (or MCP-aware agent) session. Installs a global skill + a local daemon that picks tasks off a shared bind-mounted dir, runs them via `run_claude.sh` (per-task model routing, budget caps, permission scoping), and returns results — no inbound network, no tunnel. Survives reboots, idempotent. MIT. |
