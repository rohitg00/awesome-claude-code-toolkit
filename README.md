...
| [claude-slim](https://github.com/iops-leo/claude-slim) | 42+ | Audits what your Claude Code startup context actually costs. Measures every skill, plugin, agent, command, and memory file in `~/.claude/` with js-tiktoken, then reversibly disables the dead weight — unused skills, duplicate registrations, broken symlinks, stale memory. Tiered proposals, `--dry-run`, one-command restore. No proxy, no compression. `npx claude-slim scan` |
