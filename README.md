...
| [somework/dsx](https://github.com/somework/dsx) | new | git-style sync between a Claude Design project and a local directory (`clone`/`pull`/`push`/`status`/`diff`/`fetch`), without the file bytes ever passing through a model's context — a pull an agent would spend a lot of tokens reading costs dsx one summary line. Reads Claude Code's existing OAuth token, no separate login or MCP wiring. Single Go binary, standard library only, macOS + Linux, MIT |
