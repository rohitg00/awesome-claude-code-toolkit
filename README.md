...
| [recap-studio](https://github.com/Aboudjem/recap-studio) | Turns a topic or a coding session into a self-contained, offline, dark-mode one-page HTML explainer with cited sources. No JavaScript output, works on a plane. MIT |
| [aws-cost-audit-skill](https://github.com/Aboudjem/aws-cost-audit-skill) | Audits a live AWS account read-only. Attributes every dollar, finds idle and orphaned resources, checks Savings Plans and RI coverage, and ships a savings plan with confidence levels and reversible, gated actions. Verifies prices live, hardcodes none. MIT |
| [goalify](https://github.com/Aboudjem/goalify) | Scopes a big coding task, locks the few real decisions, and writes a self-contained goal file so a fresh session executes the whole job and verifies every success criterion before calling it done. Survives /clear. MIT |
| [loopify](https://github.com/Aboudjem/loopify) | Writes a standing brief and one line for Claude Code's built-in /loop, so a job that repeats, like babysitting a release PR or sweeping bug reports, runs on an interval and logs every tick. MIT |
...
| [10x](https://github.com/Aboudjem/10x) | -- | A Claude Code plugin marketplace of 7 plugins: sniff, ui-ux-suite, recap-studio, aws-cost-audit, goalify, humanizer, loopify. One install: `/plugin marketplace add Aboudjem/10x` |
