...
| [claude-pin](https://github.com/OmarNassar1127/claude-pin) ([npm](https://www.npmjs.com/package/claude-pin)) | Pin Claude Code sessions so they survive the `--resume` window. `/pin`, `/unpin`, `/pins`, `/note` slash commands plus an interactive `cpin` CLI picker (↑/↓ navigate, space select, enter resume, `/` to search, `d` to unpin). Resumes by session id, bypassing the picker filter entirely. Zero dependencies, MIT. Install: `npm i -g claude-pin` |
