...
| [inmemory](https://github.com/Reikor-Arg/inmemory) | Verbatim recall across sessions, built from the transcripts Claude Code already writes -- BM25 over a hand-rolled inverted index, zero dependencies, no API calls, nothing leaves the machine. When the context is compacted it hands back your own turns in order rather than a second summary. Also declines a re-read of a file that has not changed since the last one, and an oversized skill load. Silent when nothing matches. MIT, tested on Linux, macOS and Windows. |
