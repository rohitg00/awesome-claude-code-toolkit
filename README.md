...
| [lemmaly](https://github.com/morsechimwai/lemmaly) | `npx skills add morsechimwai/lemmaly` | Four composable skills — `lemmaly` (gateway: state Big-O, name data structure, pick algorithm), `invariant-guard` (prove termination, base cases, edge tables), `mathguard` (reach for Bloom/HLL/Count-Min/FFT/LSH when classical hits its floor), `complexity-cuts` (corrective playbook for shipped code: nested loop → hash, repeated scan → index, brute-force → sort+sweep). Ships a deterministic CI scanner with 59 rules across 11 languages (JS/TS, Python, SQL, Java, C#, Go, Rust, C++, PHP, Ruby, Bash). No LLM, no API key, no network. Apache-2.0. |
