...
| [agent-workspace-architecture](https://github.com/jimy-r/agent-workspace-architecture) | -- | Redacted reference architecture of a production Claude Code workspace run as a governed system -- pure-role persona library, classify-then-act background agent with review queue, weekly self-audit with canaries, dead-man's switch on scheduled jobs, token-budget governance, interactive GitHub Pages tour |
