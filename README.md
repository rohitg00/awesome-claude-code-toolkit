...
| [claude-speed](https://github.com/JuDaXia/claude-speed) | new | macOS menu bar app + cross-platform statusline showing Claude Code's true generation speed (tok/s) and first-token latency as separate numbers, via Theil-Sen regression over local session transcripts. Live waiting counter for pending requests, API error surfacing, prompt-cache hit rate, per-model fitting. Zero dependencies, no network access, MIT |
