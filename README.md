...
| [ThumbGate](https://github.com/IgorGanapolsky/ThumbGate) | Local-first MIT npm CLI that ships PreToolUse / beforeMCPExecution hooks across Claude Code, Cursor, Codex, Gemini CLI, Amp, Cline, OpenCode. Blocks dangerous tool calls (`rm -rf`, force-push, secret writes) before they execute. A 👎 thumbs-down on a blocked action becomes an auto-promoted prevention rule that holds across every session, model, and agent. Install: `npx thumbgate init`. |
