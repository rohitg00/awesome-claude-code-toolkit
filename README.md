...
| [Tokens 4 Breakfast](https://www.tokens4breakfast.app) | new | Menu bar tracker for token usage, spend, subscriptions & Claude Code rate-limit pressure across 8 providers (Claude Code, OpenAI, Cursor, Copilot, Gemini, DeepSeek, Mistral, OpenRouter). Get Claude Code usage insights per project and per model. Local-first, private, offline, no login, no cloud, no subscriptions |
