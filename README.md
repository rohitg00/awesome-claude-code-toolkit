...
| [Sillage](https://github.com/MarlBurroW/sillage) | new | Self-hosted, mobile-first web UI that drives the native Claude Code and Codex CLIs on your own machine. Sessions that outlive the client, full-text search over every conversation, an IDE panel (file explorer, editor, diffs, terminal), worktrees, a board the agents read through its own MCP server, and an installable PWA with push. Single Docker container. TypeScript, MIT |
