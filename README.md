...
| [Hivemind](https://github.com/activeloopai/hivemind) | 1,500+ | Captures coding-agent sessions via lifecycle hooks, then a background worker distills traces into SKILL.md files under `.claude/skills/` that Claude Code loads natively, so fixes stop getting rediscovered. Team workspaces share skills so one engineer's lesson loads for the whole team. Also works with Codex, Cursor, OpenClaw. Self-hostable storage (S3, GCS, Azure). Install: `curl -fsSL https://deeplake.ai/hivemind.sh \| sh` |
