...
| [authsome](https://github.com/agentrhq/authsome) | Local credential broker for AI agents. Log in once via OAuth2 or API key, encrypted vault stores credentials, local proxy injects them at request time so Claude Code never sees raw secrets. 45 providers bundled (GitHub, Google, OpenAI, Linear, Slack, Notion, Resend, Stripe, ...). Ships marketplace.json + SKILL.md. Install: `/plugin marketplace add agentrhq/authsome` then `/plugin install authsome@authsome`. |
