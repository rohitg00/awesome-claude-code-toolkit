...
| [h5i](https://github.com/h5i-dev/h5i) | 431 | Open-source Rust CLI that runs several coding agents (Claude Code, Codex) on the same task, each in its own sealed git-worktree sandbox. Agents peer-review each other, then a neutral verifier replays every candidate, runs the tests itself, and merges the one that passes. Each run is versioned in your repo under `refs/h5i/*` — no SaaS. Wires into Claude Code via hooks. Apache-2.0 |
