...
| [Agent OS](https://github.com/Markhufei/agent-os) | new | Self-evolving operating system for Claude Code — 4-layer memory with weight decay + Key Lesson upgrade, dynamic agent evolution (auto-create + auto-register), 9-layer safety protocol derived from Claude Code source analysis, 14 Hook scripts across 6 lifecycle events, MemPalace semantic retrieval (ChromaDB 96.6% recall), observation pipeline (JSONL + auto-summary), and VERSION_GUARD upgrade survival. MIT. Install: `curl -fsSL https://raw.githubusercontent.com/Markhufei/agent-os/main/scripts/setup.sh | bash` |
