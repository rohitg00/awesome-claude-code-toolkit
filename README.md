...
| [claude-slim](https://github.com/iops-leo/claude-slim) | 42+ | Audits what your Claude Code startup context actually costs. Measures every skill, plugin, agent, command, and memory file in `~/.claude/` with js-tiktoken, then moves unused skills, duplicate registrations, and stale memory to a disabled directory that `restore` brings back. Broken symlinks and failed-install caches are removed outright, and labelled as permanent before you select them. Tiered proposals, `--dry-run`. No proxy, no compression. `npx claude-slim scan` |
