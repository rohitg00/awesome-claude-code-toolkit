...
| [WaitPerk](https://github.com/sammpentz-commits/waitperk-client) | new | Sponsored status line for Claude Code. One sponsor message renders in the status bar and developers are paid for the verified time it is on screen, with sponsor payments split 50/50 with the developer pool. Single-file stdlib-only Python client, Ed25519-signed self-updates, byte-exact uninstall. Source-available at the linked repo; live numbers at waitperk.com |
