...
| [ThumbGate](https://github.com/IgorGanapolsky/ThumbGate) | Local-first PreToolUse firewall that blocks dangerous tool calls (`rm -rf`, secret writes, off-scope edits, bad `git push`) and turns a 👎 into an auto-promoted prevention rule that persists across sessions -- so repeat mistakes are blocked automatically instead of re-configured by hand. A learning layer on top of static hooks. MIT, `npx thumbgate`. Works across Claude Code, Cursor, Codex, Gemini, Amp, Cline, OpenCode |
