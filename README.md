...
| [ChiefLab](https://chieflab.io) | -- | After-build growth loop for coding agents. Reference impl of [After-Build Hook v0.1](https://chieflab.io/spec/after-build-hook) (MIT). MCP at api.chieflab.io. Trigger: user says "get users" / "launch this" after build → ChiefLab picks ONE channel for the product type, drafts the post, founder posts manually, ChiefLab measures at 24h, creates tomorrow's move. Manual-execution-first; no connector required for the first loop |
