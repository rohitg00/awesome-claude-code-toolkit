...
| [deploy](https://github.com/mturac/deploy) | Multi-platform deployment for Claude Code. Detects your framework automatically and deploys to Vercel, Railway, Fly.io, or Render with preview-before-production enforcement, rollback safety with DB migration checks, and build failure troubleshooting. |
...
| [incident](https://github.com/mturac/incident) | Production incident response in Claude Code. Triage errors from Sentry, Datadog, or stack traces. Hypothesis-driven diagnosis, root cause analysis, fix generation, and blameless postmortems with 5-Why RCA. Optional MCP server for direct Sentry/Datadog API access; MCP API access is read-only. |
