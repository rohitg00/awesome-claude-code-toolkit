...
| [AgentGrid](https://agentgrid.sh) | - | Infinite zoomable canvas desktop app (Electron, macOS/Linux/Windows) for orchestrating multiple Claude Code agents in parallel. A master orchestrator pane spawns role-based workers (builder, QA, reviewer) over an MCP tool surface, each in its own git worktree. Integrated terminals and Playwright-driven browser panes live on the same canvas. Also runs the OpenAI Codex SDK. Free tier available |
