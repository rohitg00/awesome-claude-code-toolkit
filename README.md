...
| [rote](https://github.com/trevhud/rote) | 3 | Compiles a proven Claude skill (a SKILL.md plus references) into a typed, deterministic pipeline. Fixed logic becomes reviewable Python or TypeScript with per-step tests, while judgment steps stay as typed LLM-judge signatures. Emits DBOS, Temporal, Cloudflare Workflows, Inngest, or plain Python/TS, and `rote serve` exposes compiled pipelines back to Claude as MCP tools. Ships a Claude Code plugin: `/plugin marketplace add trevhud/rote` then `/plugin install rote@rote`. Apache-2.0. `pip install rote-cli` |
