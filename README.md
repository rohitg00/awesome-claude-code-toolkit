...
| [CLAUDE.md Auditor](https://github.com/infai-tech/claudemd-auditor-skill) | `git clone https://github.com/infai-tech/claudemd-auditor-skill && cp -r claudemd-auditor-skill/claudemd-auditor ~/.claude/skills/` | Calibrated 0–100 scoring for `CLAUDE.md` files on six axes (Specificity, Coverage, Brevity, Currency, Quirks captured, Tone fit). v1.0 matches human baselines at Δ=0 across a 6-case corpus spanning 14–78 points; adversarial-input corpus exercises all three error codes (`too_short`, `not_a_claudemd`, `too_long`). Returns paste-ready Top-3 improvements. MIT. |
