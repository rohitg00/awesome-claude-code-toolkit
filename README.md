...
| [argus-audit](https://github.com/mturac/pluginpool-argus-audit) | Scope-gated white-hat security audit + pentest orchestrator. Cryptographic ownership-proof vault (refuses to scan without target-bound HMAC token), self-updating intel layer (NVD / KEV / EPSS / arxiv), five scanner surfaces (Python AST taint, secrets with SHA redaction, supply-chain incl. GHA permissions audit, TLS handshake, HTTP probes with day-one CVE detectors). Stdlib-only, 174 hermetic tests. Install: `/plugin marketplace add mturac/claude-plugin-marketplace` then `/plugin install argus-audit`. |
| [commit-narrator](https://github.com/mturac/pluginpool-commit-narrator) | Conventional commit message generator from `git diff --staged`. Handles git-quoted filenames containing spaces or escaped quotes. Stdlib-only Python. |
| [pr-storyteller](https://github.com/mturac/pluginpool-pr-storyteller) | PR title + body + test plan generated from commits/diff vs base branch. |
| [test-gap](https://github.com/mturac/pluginpool-test-gap) | Surface git diff lines that aren't covered by tests. State-machine diff parser, Cobertura + LCOV + coverage.py JSON, merges duplicate `<class>` entries and honors `<sources>` roots. |
| [deps-doctor](https://github.com/mturac/pluginpool-deps-doctor) | Dependency vuln/audit across npm, pip, cargo, go. Monorepo-aware recursive detection, fail-loud on non-JSON tool output (no silent green), real OSV `fixed_in` extraction for govulncheck. |
| [standup-gen](https://github.com/mturac/pluginpool-standup-gen) | Daily standup notes generated from git activity across one or many repos. |
| [todo-harvest](https://github.com/mturac/pluginpool-todo-harvest) | TODO/FIXME/HACK scan with author + age via `git blame --porcelain`, batched one call per file. Marker regex accepts `TODO(user):`, `FIXME[ABC-123]:`, `HACK!` decorations. |
| [env-lint](https://github.com/mturac/pluginpool-env-lint) | `.env` vs `.env.example` key parity with inline-comment-safe value parsing, `--strict` mode for empty values, never prints secrets. |
| [secret-guard](https://github.com/mturac/pluginpool-secret-guard) | Pre-commit secret scanner: 12 regex patterns + Shannon entropy on long literals. Evidence is always SHA-256 redacted so reports are safe to share. |
| [flaky-detector](https://github.com/mturac/pluginpool-flaky-detector) | Run a test command N times, report per-test flakiness %. Correct `pytest -q` math via missing-run-imputation, multi-parser support (pytest / jest / gotest / TAP). |
| [changelog-forge](https://github.com/mturac/pluginpool-changelog-forge) | Group conventional commits since the last tag into a CHANGELOG section + semver bump suggestion. |
| [claude-roundtable](https://github.com/mturac/claude-roundtable) | Multi-agent governance for Claude Code. Council deliberates, votes (weighted), dispatches to specialised council agents, and enforces quality gates before tasks are accepted. |
| [skill-hunter](https://github.com/mturac/skill-hunter) | Pre-execution discovery layer. UserPromptSubmit hook that forces an agent to check for an existing skill / MCP / CLI / library before building anything from scratch. Adapters for Claude Code, Codex, OpenClaw, Hermes, and Cursor. |
| [promptguard](https://github.com/mturac/promptguard) | Audits LLM prompts and agent instructions as behavioral contracts. Pre-write guard for agents that ship code — flags vague intent, missing responsibility, false certainty, conflicting rules. |
| [moooom](https://github.com/moooom-claude/moooom-claude) | 10 cultures of moms nagging your Claude Code to drink water, eat food, and sit up straight. Culture-aware hook reads `$MOOOOM_CULTURE` (`tr`, `en`, `jp`, `mx`, `de`, `it`, `es`, `fr`, `kr`, `in`, or `random`). |
...
| [mturac/tools](https://github.com/mturac/tools) | new | Open-source toolkit for AI-augmented engineering — 11 Claude Code plugins (security audit, commit/PR/standup helpers, supply-chain auditor, secret scanner, flaky-test detector), 6 AI-agent infrastructure projects (multi-agent council, MCP-backed memory, durable scheduler, skill-hunter hook, prompt contract auditor, multi-culture wellness reminders). Single hub repo. Reviewed and tagged v0.1.0+ |
| [mturac/claude-plugin-marketplace](https://github.com/mturac/claude-plugin-marketplace) | new | Claude Code plugin marketplace bundling 15 mturac plugins (pluginpool + AI-agent infra). One-line install: `/plugin marketplace add mturac/claude-plugin-marketplace`. Each plugin lives in its own repo with own LICENSE, tests, and release tags |
| [recall-mcp](https://github.com/mturac/recall-mcp) | new | Shared brain for AI agents — SQLite-backed persistent memory over the Model Context Protocol with hybrid FTS5 + vector search, human-cognition-inspired memory decay (STRONG/MEDIUM/WEAK), namespace isolation, parameterised SQL throughout, BigInt-safe responses, constant-time auth. Available as MCP server |
| [leyla-scheduler](https://github.com/mturac/leyla-scheduler) | new | Durable, session-aware task scheduler in Rust. Cron-style triggers, leases, replace-running semantics; jobs and runs survive disconnects. SQLite-backed engine |
