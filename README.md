...
| [chamnan](https://github.com/ArcticFox2029/chamnan) | Keeps engineering context in the repository so a new session reads instead of rediscovering — an architecture index, a reverse-dependency impact map, session hand-offs that carry only what was left unfinished, decisions/lessons/rules, and captured procedures, all plain markdown committed beside the code. Session start receives names and pointers under a token ceiling, never file bodies. Hardened against a synthetic 2,365-file corpus spanning 31 file types with comments in 8 writing systems: 15 bugs found and fixed, each with a regression check, and none of 92 planted credentials reached the index. No network, no account, no telemetry, no third-party packages — Python 3.8+ stdlib, 378 tests, MIT. Install: `claude plugin marketplace add ArcticFox2029/chamnan` then `claude plugin install chamnan@chamnan` |
