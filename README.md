...
| [AI Memory Reader](https://github.com/nvwalj/ai-memory-reader) | new | Native macOS & iOS app for browsing and editing AI agent memory files. Auto-detects directories for Claude Code (`~/.claude`), OpenClaw, Codex, Cursor, Gemini, Continue, Aider, and GitHub Copilot. SwiftUI + MarkdownUI rendering, sidebar tree, in-page find (⌘F), edit mode with auto-save (⌘E), URL scheme + CLI (`aimr`) for agent integration. GPL-3.0 |
