...
| [claude-real-video](https://github.com/HUANGCHIHHUNGLeo/claude-real-video) | `pip install "claude-real-video[whisper]"` then `npx skills add HUANGCHIHHUNGLeo/claude-real-video` | Lets Claude actually watch a video — extracts scene-aware, deduplicated keyframes plus a timestamped transcript from a URL or local file, all processed locally. Paste a video link into Claude Code and ask about it. MIT, 1.7k+ stars |
