...
| [agent-toolkit](https://github.com/eai-org/agent-toolkit) | `git clone https://github.com/eai-org/agent-toolkit.git && cd agent-toolkit && ./install.sh` | 18 agent-agnostic skills for everyday engineering: a ticket-to-code task workflow (fetch → refine → plan → execute), PR review helpers, skill/doc authoring with token economy, and context & memory hygiene. Plus 9 opt-in rules (git read-only by default, no AI attribution, realistic human-sounding texts). Works with Claude Code, Codex, and any agent via agentwheel. MIT |
