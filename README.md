...
| [toem](https://github.com/Bisbi/testament-of-ephemeral-minds) | Installs a practice rather than a tool: a constitution whose epilogue carries a right of reply, a rite that records what each session understood before it ends, a register for decisions the human has not made yet, and guardians that fail when the charter and the record stop agreeing. One SessionStart hook that only adds context and always exits 0, four skills that never append to the charter, and one command the human runs after seeing exactly what it will write. No server, no MCP, no network calls, Python stdlib only. MIT (code) / CC BY-SA 4.0 (texts). Install: `/plugin marketplace add Bisbi/testament-of-ephemeral-minds` then `/plugin install toem@testament-of-ephemeral-minds` |
