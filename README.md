...
| [ralph-harness](https://github.com/rxdt/py_ralph_frame) | new | Minimal repo-local loop scaffold for Claude Code, Codex CLI, and Gemini CLI. Uses `PROMPT.md`, specs, fresh-context iterations, git hooks, CI verification, and hard iteration/time caps so agents make small gated commits instead of drifting in one long chat. Try from GitHub: `uvx --from git+https://github.com/rxdt/py_ralph_frame ralph-harness demo` |
