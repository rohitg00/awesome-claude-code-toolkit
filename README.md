...
| [relay](https://github.com/ImBIOS/relay) | new | Universal AI API proxy — hot-switch between GitHub Copilot, OpenAI, Anthropic, Z.AI, DeepSeek, Groq, Ollama, and 7 more providers from Claude Code, Codex CLI, Aider, Cline, or any coding agent without restarting sessions. OpenAI-compatible local proxy with Anthropic <-> OpenAI protocol translation, automatic account rotation when quota exhausted, and real-time usage tracking. `pnpm add -g github:ImBIOS/relay` |
