...
| [LaunchMyStore](https://github.com/LaunchMyStore/skill) | `npm i -g @launchmystore/claude-skill` or `git clone https://github.com/LaunchMyStore/skill ~/.claude/skills/launchmystore` | Natural-language router for the LaunchMyStore e-commerce platform — scaffold apps, ship checkout/admin/post-purchase extensions, deploy WASM Functions (discount/shipping/cart-transform/payment/delivery/order-validation), manage products/orders/customers via the LaunchMyStore MCP (130+ tools), and write Aqua/Liquid theme code. Universal SKILL.md — also works in Codex, Cursor, Gemini CLI, Windsurf, Antigravity, Aider, OpenCode, Kilo Code, Augment, Hermes, Mistral Vibe. MIT |
