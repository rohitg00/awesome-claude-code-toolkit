...
| [geo-check](https://github.com/vasco-branco06/geo-check) | `git clone https://github.com/vasco-branco06/geo-check ~/.claude/skills/geo-check` | Audits whether AI crawlers can reach and read a site, and scores it. Two scores, never averaged: Access and Readability, each point traceable to the robots.txt line or the markup that earned it, with no model anywhere in the scoring path. Separates training crawlers from citation crawlers, so blocking GPTBot costs nothing while blocking OAI-SearchBot removes the site from ChatGPT results. Emits the exact robots.txt lines to paste for every failed check. Validated across 906 real sites. Also ships as a CLI and a GitHub Action. MIT |
