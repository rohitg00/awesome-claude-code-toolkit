...
| [recap](https://github.com/noluyorAbi/claude-code-recap) | `npx claude-code-recap` | Session manager for Claude Code that lists recent sessions across every project, not just the current directory, with a ready-to-paste `claude -r` resume command; `--pick` jumps into one, `--open` reopens a whole working set in terminal tabs. Pure Python stdlib, zero dependencies, offline and read-only. Ships as a skill, a plugin, and the npm package claude-code-recap. |
