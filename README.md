...
| [dev-report](https://github.com/delpicorp/dev-report) | `/plugin marketplace add delpicorp/dev-report` then `/plugin install dev-report@delpicorp` | Explains a Claude Code session in plain language: what changed, why that approach was chosen, what is still unfinished or unverified, and what to do next. Every technical term is explained at first use. Korean, Japanese, and Spanish command aliases included |
