...
| [Caprock](https://github.com/dspv/caprock) | 4 | Local mission control for Claude Code. Captures every session on the machine via hooks + transcript tailing into one SQLite event stream: live activity, cost per repo, token burn, loop alerts, full-text search over what Claude actually wrote, plus spawning and typing into sessions. Single static Go binary, loopback only, no telemetry, no account. Apache-2.0 |
