...
| [RigLine](https://github.com/ZhouYinLong-lab/Dog-RigLine) | 🐕 Pure Bash Claude Code statusline with real-time GPU monitoring (utilization, VRAM, temperature via nvidia-smi), CPU & RAM (Win/Linux/macOS native), context window progress bar, token tracking, CJK-aware column padding. Zero dependencies (bash 4+ + jq). 130+ icons with 5 theme presets. 7 snapshot tests + CI on Ubuntu/macOS/Windows. MIT. |
