...
| [fable-overclock](https://github.com/plugtheliam/fable-overclock) | `git clone https://github.com/plugtheliam/fable-overclock && fable-overclock/harness/install.sh` | Per-model reliability harness — makes the model you run (Opus/Sonnet/DeepSeek) cite every figure, say UNKNOWN instead of guessing, and cross-check claims with a second model. Ships as a skill + an optional deterministic PreToolUse gate that flags unsourced/fabricated numbers. stdlib-only, non-blocking, `--uninstall` reverses it. |
