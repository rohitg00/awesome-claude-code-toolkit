...
| [claude-code-account-switcher](https://github.com/Nemo-Illusionist/claude-code-account-switcher) | new | Binds Claude Code accounts to directories -- `cd` activates the matching account via `CLAUDE_CONFIG_DIR`, with inheritance down the directory tree, per-account settings/CLAUDE.md/agents, a wrapper so JetBrains and VS Code launches resolve the same binding, 5h/7d usage per account, a status line showing the active account, and a `doctor` command that audits which identity each config dir actually resolves to. Rust CLI plus a single-file zsh variant. macOS, Linux, Windows. MIT |
