...
| [capcut-edit](https://github.com/renezander030/capcut-cli) | `/plugin marketplace add https://github.com/renezander030/capcut-cli` | Edit CapCut and JianYing video drafts (`draft_content.json`) from the CLI. Subtitles, timing, audio, decorators (keyframes, masks, transitions, text/image animations), template apply, long-form-to-shorts cut. Whisper → real caption objects (v0.4), version migrate/lint/decrypt, multi-language draft clone. Zero deps (Node only), both CapCut + JianYing namespaces in one binary. MIT. |
