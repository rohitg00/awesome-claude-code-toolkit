...
| [Sum-Usage-Claude](https://github.com/ArcticFox2029/Sum-Usage-Claude) | new | Offline dashboard for what your Claude Code usage would have cost at API list prices, against what you actually paid. Reads the transcripts Claude Code already writes — no install, no package, no server: the build inlines the data so `index.html` opens straight from `file://`. Real billing cycles rather than calendar months, so the multiple is against the invoice you were sent. Deduplicates by `requestId` (48% of records were repeats), prices cache reads at 0.1× input separately, and buckets by local day instead of UTC. Thai and English. Python 3 stdlib only, MIT |
