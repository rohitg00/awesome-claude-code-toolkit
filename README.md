...
| [claude-code-eyes](https://github.com/fcavalcantirj/claude-code-eyes) | `git clone https://github.com/fcavalcantirj/claude-code-eyes ~/.claude/skills/claude-code-eyes` | Give Claude Code eyes — look at real hardware, displays, and wiring through a camera (Android IP Webcam, Pi cam, or any snapshot URL) to visually verify a rendered panel or check wiring before power-on. Catches bugs that live on the glass, not in the logs. ~100 lines of bash, MIT, one-command setup. |
