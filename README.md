...
| [verdict](https://github.com/ArtJack/verdict) | Skeptical QA agent with memory -- baseline then delta runs (NEW/REGRESSED/RESOLVED), every red test classified, flaky tests quarantined with an expiry, evidence-cited verdicts. Never edits your code (no Edit tool, write-scope hook, strict Bash guard). Ships a read-only MCP server over the QA state, an exit-code CI release gate, and a scored eval suite with published misses. Python stdlib only, MIT |
