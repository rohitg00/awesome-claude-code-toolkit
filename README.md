...
| [operating-kit](https://github.com/Sharrmavishal/operating-kit) | new | Portable, project-agnostic operating method for Claude Code and Cursor. Ships builder + strategic-vetting persona, vigilance protocol, multi-model collaboration, silent-failure field notes, incident response, 5 subagent templates (session-start/end, code-review, deploy, prod-logs), memory seeds, and Cursor `.mdc` adapter. Self-installs via one paste. MIT. |
