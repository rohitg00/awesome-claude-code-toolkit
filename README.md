...
| [claude-code-skills](https://github.com/Zavelinski/claude-code-skills) | One-line plugin marketplace of 20 focused Claude Code skills, plugins, and MCP servers, each grounded in a study or measured failure mode: context-rot defense (context-warden), cost-aware model routing (model-router), adversarial test-gen (testsmith), MCP vetting (mcp-warden), a zero-dependency cross-session memory MCP (recall), plan/spec/build/verify orchestration, and more. MIT. Install: `/plugin marketplace add Zavelinski/claude-code-skills` |
