...
| [skill-loom](https://github.com/michelepastorello/skill-loom) | `npx skill-loom` | One git-versioned vault of agent skills, symlinked into every runtime (Claude Code, Codex, custom agents) — edit once, every agent sees the same skill. Dry-run by default, never overwrites non-symlink files, ships status/doctor/migrate |
