...
| [Claude Code Hooks Settings Checker](https://claude-code-hooks-settings-checker.vercel.app/) | new | Browser-only checker for Claude Code `settings.json` hooks. Paste settings locally to inspect hook events, matchers, handler fields, and config placement; no signup, uploads, analytics, or external scripts. Open source. |
