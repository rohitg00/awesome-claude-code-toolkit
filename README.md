...
| [Ophis](https://github.com/ophis-fi/skills) | `/plugin marketplace add ophis-fi/skills && /plugin install ophis@ophis-fi` | Intent-based DEX swaps for AI agents, a CoW Protocol deployment. Bundles a remote MCP server (12 tools, no API key) plus the ophis-swap workflow skill. Swaps are MEV-protected, gasless for the trader (no native gas coin needed), keyless, non-custodial (the agent signs its own order and the receiver is pinned to the signer), and rebate-eligible across 11 EVM chains (Ethereum, Optimism, BNB Chain, Gnosis, Polygon, Base, Arbitrum, Avalanche, Plasma, Ink, Linea). Not njayp/ophis (an unrelated Go MCP library); this is Ophis the DEX (ophis.fi). MIT. |
