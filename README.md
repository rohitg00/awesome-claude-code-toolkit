...
| [c9s](https://github.com/StefanoGuerrini/c9s) | new | Terminal dashboard for Claude Code sessions, inspired by k9s. Shows every session across every project in one view, opens each one in its own tmux window, and surfaces live status (processing / waiting / done) from Claude Code hooks. Single Go binary plus tmux, no daemon, reads `~/.claude/` directly. macOS + Linux, MIT. Homebrew: `brew install stefanoguerrini/tap/c9s`. |
