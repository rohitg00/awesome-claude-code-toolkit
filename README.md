...
| [argot](https://github.com/get-tmonier/argot) | Plugin bundling 5 skills, an MCP server, and an opt-in non-blocking pre-write hook that asks before an agent adds a dependency the repo has never used. Flags code foreign to a repo's own git-history-learned patterns — unfamiliar dependencies, reinvented functions, misplaced code, module-layering breaks, and tests weakened, disabled, or deleted alongside a production change. Single Rust binary, 100% local, no LLM. Install: `/plugin marketplace add get-tmonier/argot` |
