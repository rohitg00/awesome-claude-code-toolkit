...
| [context-bridge](https://github.com/serdardb/context-bridge) | new | Context bridge across Claude Code, Codex, Grok and Antigravity sessions. Each agent keeps its own native session; the bridge records which sessions belong to a project and injects only the delta the target is missing, in any of the twelve directions. Drives the subscription-authenticated CLIs, no API keys. Node, zero dependencies, MIT. `npx @serdardb/context-bridge` |
