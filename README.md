...
| [PocketAI](https://apps.apple.com/us/app/pocketai-remote-coding-agent/id6787897526) | - | Native iOS client for driving the Claude Code on your own machine (macOS / Windows-WSL / Linux). Permission prompts as tappable cards with red/green diff previews, streaming chat with thinking blocks and tool cards, multi-machine and multi-session, works over cellular via encrypted relay (both ends outbound-only, no port forwarding). Free tier; auth-agnostic (subscription, API key, or gateway) |
