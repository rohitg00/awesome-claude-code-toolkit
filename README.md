...
| [hush](https://github.com/V-Songbird/hush) | Keeps Claude quiet until the job is done: one short answer at the end that leads with the result and names the file to open, plus hooks that trim long command output and logs before they pile up in the session. Install: `/plugin marketplace add V-Songbird/foundry` |
| [razor](https://github.com/V-Songbird/razor) | Stops Claude from adding code nobody needed: one evidence-backed "do we need this?" check before every new package, file or abstraction, and a gate on package installs. Install: `/plugin marketplace add V-Songbird/foundry` |
| [foreman](https://github.com/V-Songbird/foreman) | Keeps the project plan next to the code as a plain roadmap file: ask what's next and get the task, why it comes first, and a ready-to-run prompt checked against the real files. Install: `/plugin marketplace add V-Songbird/foundry` |
