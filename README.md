...
| [aigent-OS](https://github.com/wrg32786/aigent-os) | new | Full open-source Claude Code operating system -- a markdown + shell kernel that gives Claude Code persistent memory (an Obsidian vault), a self-model, a session /open + /close rhythm, a Pantheon of subagents, context hooks, and background daemons. No database, no server, no build step -- drop the files in and your AI boots up knowing what it's working on. One-command install. MIT |
