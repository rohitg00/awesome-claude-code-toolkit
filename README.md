...
| [ThumbGate](https://github.com/IgorGanapolsky/ThumbGate) | Local-first PreToolUse firewall for AI coding agents. Blocks dangerous actions (`rm -rf`, secret writes, off-scope edits, force-push to main, cloud-egress of repo files) before they run, and turns a thumbs-down into an auto-promoted prevention rule that blocks the repeat across sessions and agents. Independent Critic/Rubric PostToolUse auto-judge captures structured 👎 without a human click. Works with Claude Code, Cursor, Codex, Gemini, Amp, Cline, OpenCode. SQLite+FTS5 lesson DB, Thompson Sampling, LanceDB vectors. MIT, `npx thumbgate init`. |
