...
| [nrflo](https://github.com/nrflo/nrflo) | Self-hosted control plane that orchestrates Claude Code, Codex, Gemini and Opencode CLIs through layered workflows. Multi-phase workflows with parallel agents per phase, verifier agents that can callback an earlier phase with notes, isolated git worktrees per ticket with auto-merge, browser-based PTY session takeover, and automatic context-exhaustion recovery. Single Go binary + SQLite, uses your existing CLI subscription (no API keys). Apache 2.0. |
