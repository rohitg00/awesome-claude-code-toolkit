...
| [claude-session-sync](https://github.com/defi0x1/claude-session-sync) | new | Resume a Claude Code session on a different machine. Syncs session transcripts through a private Git repo you own via SessionStart/Stop/SessionEnd hooks — opt-in per session, retroactive sync of older sessions, optional age encryption, per-project path rewriting. POSIX shell; Git is the only runtime dependency, and setup uses python3 or jq to edit settings.json (otherwise it prints the edit to make by hand). macOS, Linux, and Windows via Git Bash. MIT |
