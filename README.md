...
| [usage-limits](https://github.com/ridelink0/claude-code-usage-limits) | Plugin that puts the remaining 5-hour and weekly limits in front of Claude before every prompt, as turns of headroom rather than a percentage, and shows them as live bars beside the chat: a split-pane panel, a status line in Claude Code's own colours, and a VS Code view under the chat, all from the same call Claude Code makes for /usage. Reads Codex too. MIT, zero dependencies. |
